...
impl HitCounter {
        ...
        pool: DatabasePool) -> Result<(), HitCountError>
    {
        ...
        handle.block_on(async move {
            ...
            for (shortcode, hits) in hits {
                if let Err(e) = service::action::increase_hit_counter(&shortcode, hits, &pool).await {
    ...
    fn process_msg(msg: HitCountMsg, hits: HitStore,
        handle: Handle, pool: DatabasePool) -> Result<(), HitCountError> {
            match msg {
                    ...
                    Err(e) => match e {
                        TryRecvError::Empty => {
                            ...
                            if let Err(e) = tx_clone.send(HitCountMsg::Commit) {
                                eprintln!("Error Sending Commit {}", e)
                            }
                        },
                        _ => break,
                    }
