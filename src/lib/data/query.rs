...
use rocket::http::hyper::Executor;
use sqlx::{pool, Row};

use super::model;
use crate::data::{DataError, DatabasePool};
use crate::ShortCode;
use crate::web::api::{ApiKey, self};
...
pub async fn save_api_key(api_key: ApiKey, pool: &DatabasePool) -> Result<ApiKey> {
    ...
    let _ = sqlx::query!("INSERT INTO api_keys (api_keys) VALUES (?)", bytes)
        .execute(pool)
        .await
        .map(|_| ())?;
    Ok(api_key)
...
pub async fn revoke_api_key(api_key: ApiKey, pool: &DatabasePool) -> Result<RevocationStatus> {
    let bytes = api_key.clone().into_inner();
    Ok(
        sqlx::query!("DELETE FROM api_keys WHERE api_keys == ?", bytes).execute(pool).await
...
pub async fn api_key_is_valid(api_key: ApiKey, pool: &DatabasePool) -> Result<bool> {
    let bytes = api_key.clone().into_inner();
    Ok(
        sqlx::query("SELECT COUNT(api_keys) FROM api_keys WHERE api_keys = ?")
        .bind(bytes).fetch_one(pool).await.map(|row| {
