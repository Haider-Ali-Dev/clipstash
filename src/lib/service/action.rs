...
use crate::data::{query, DatabasePool, Transacation};
...
pub async fn begin_transaction(pool: &DatabasePool) -> Result<Transacation<'_>, ServiceError> {
    Ok(pool.begin().await?)
}
pub async fn end_transaction(transaction: Transacation<'_>) -> Result<(), ServiceError> {
